...
export const setSpace = (args, force) => {
  ...
  switch (pos) {
    ...
    case 'h':
      return fluidify([`${properties[prop]}-left`, `${properties[prop]}-right`], space[size][0], space[size][1], isImportant);
    case 'v':
      return fluidify([`${properties[prop]}-top`, `${properties[prop]}-bottom`], space[size][0], space[size][1], isImportant);
    default:
