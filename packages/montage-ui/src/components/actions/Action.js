...
// }) => <Element {...props} />)`
//   align-items: center;
//   background: ${({ primary, theme }) => (primary ? theme.color.action : 'transparent')};
//   border-color: ${({ theme }) => theme.color.action};
//   border-radius: ${({ iconic, theme }) => (iconic ? theme.feature.roundness.a : theme.feature.roundness.action)};
//   border-style: solid;
//   border-width: ${({ theme, plain }) => (plain ? '0' : theme.box.border)};
//   box-shadow: ${({ isDisabled, plain, theme }) => (isDisabled || plain ? 'none' : `0 1px 3px ${theme.color.shadow400}`)};
//   color: ${({ primary, theme }) => (primary ? theme.color.bodyBackg : theme.color.action)};
...
//
//   /* States */
//   ${({ isDisabled, primary, theme }) => (!isDisabled
//     ? `
//     &:hover {
